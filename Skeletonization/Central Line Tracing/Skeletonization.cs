...
namespace Central_Line_Tracing
{
    public class Skeletonization
    {
        ...
        public Skeletonization(Image _image)
        {
            this.image = _image;
        }

        public bool[][] FromImageToBool()
        {
            Bitmap bitmap = new Bitmap(image);

            bool[][] s = new bool[bitmap.Height][];
            for (int y = 0; y < bitmap.Height; y++)
            {
                s[y] = new bool[bitmap.Width];
                for (int x = 0; x < bitmap.Width; x++)
                    s[y][x] = bitmap.GetPixel(x, y).GetBrightness() < 0.3;
            }
            return s;
        }
        public  Image FromBoolToImage(bool[][] s)
        {
        ...
        public int step(int stepNo, bool[][] temp, bool[][] s)
        {
            int count = 0;
        ...
        public bool SuenThinningAlg(int x, int y, bool[][] s, bool even)
        {
            bool p2 = s[x][y - 1];
            bool p3 = s[x + 1][y - 1];
            bool p4 = s[x + 1][y];
        ...
        public int NumberOfZeroToOneTransitionFromP9(int x, int y, bool[][] s)
        {
            bool p2 = s[x][y - 1];
            bool p3 = s[x + 1][y - 1];
            bool p4 = s[x + 1][y];
        ...
        public int NumberOfNonZeroNeighbors(int x, int y, bool[][] s)
        {
            int count = 0;
            if (s[x - 1][y]) count++;
            if (s[x - 1][y + 1]) count++;
